...
containsDiracHole pip =
  let chargePolyS5 = spreadPoly 5
      ...
      entries = multisetToList pip.maxelMap
...
vacuumAnnihilation pip =
  if containsDiracHole pip then
    let inverseSupp = negateMultiset pip.maxelMap
        restoredSupp = annihilateMultiset (addMultiset pip.maxelMap inverseSupp)
    in MkSparseMaxel restoredSupp
  else
    pip
