...
| 📉 **[Probability Bounds](https://github.com/justinkelly-ie/idris2-Universe-Wiki/blob/main/Library/Wiki/Finite-Science/Probability_Bounds.md)** (Hailperin) | Min/max corner msets <span>$\langle \text{Box}_\text{Min}, \text{Box}_\text{Max} \rangle$</span> | Universe constraints $\{\sum w \cdot [\text{bounds}]\}$ | Hailperin co-operations — multiset boundary scaling | Inclusion-exclusion corners (alternating sign sequences / polyhedral vertex squeezing) | `idris2-Boole` 📄 `Hailperin.idr` |
