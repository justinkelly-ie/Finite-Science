...
|---|---|---|---|---|---|---|
| 🎛️ **[Singleton Repetition](https://github.com/justinkelly-ie/idris2-Universe-Wiki/blob/main/Library/Wiki/Finite-Science/Singleton_Repition.md)** (Algebra of Boole - Singleton Variant) *(Digital Repetition is Deprecated)* | Singleton Bit-gate multiset $\{b \cdot [\text{state}]\}$ (size 0 or 1) | Strictly positive unit constant $\{1 \cdot [\text{BaseAnchor}]\}$ (size 1) | Relational logic operator (multiset of logic transition relations) | Modulo-2 wiring connections (multiset of connection routes) | `[1]` (modulo-2 parity collapse/identity form) | `idris2-Boole`, `idris2-Multiset` (`BF2.idr`, `SingFraction.idr`, `Transformation.idr`, `SBFMset.idr`, `Sing.idr`, `Sing1.idr`) |
| 💻 **[Singleton Lifted Polynumbers](https://github.com/justinkelly-ie/idris2-Universe-Wiki/blob/main/Library/Wiki/Finite-Science/Singleton_Lifted.md)** (Arithmetic Circuits - Singleton Variant) *(Lifted Polynumbers is Deprecated)* | Singleton truth table coefficients $\{w \cdot [\text{state}]\}$ (size 0 or 1) | Strictly positive unit constant $\{1 \cdot [\text{BaseAnchor}]\}$ (size 1) | Boole-Möbius shifts (multiset of transition relations with BoxInt weights) | Modulo-2 wiring connections with BoxInt weights | `[1, 2, 4, 8, ...]` (positional binary scaling/carry form) | `idris2-Boole`, `idris2-Multiset` (`BoxInt.idr`, `SingFraction.idr`, `Transformation.idr`, `SBFMset.idr`, `Sing.idr`, `Sing1.idr`) |
| 🧮 **[Functional Probability](https://github.com/justinkelly-ie/idris2-Universe-Wiki/blob/main/Library/Wiki/Finite-Science/Functional_Probability.md)** (Hehner) | Quantified mset $\{w \cdot [\text{valid}]\}$ | Total universe sum $\{\sum w \cdot [\text{all states}]\}$ | Hehner normalization $\overline{E} = E \cdot (\sum E)^{-1}$ | Min / max evaluation (calculational quantifiers $\land$ and $\lor$ replacing infinite logic) | Normalizing inner product $\langle A, B \rangle = \sum A_i B_i$ | `idris2-Boole` 📄 `FunctionalProbability.idr` |
| 📉 **[Probability Bounds](https://github.com/justinkelly-ie/idris2-Universe-Wiki/blob/main/Library/Wiki/Finite-Science/Probability_Bounds.md)** (Hailperin) | Min/max corner msets $\langle \text{Box}\_{\text{Min}}, \text{Box}\_{\text{Max}} \rangle$ | Universe constraints $\{\sum w \cdot [\text{bounds}]\}$ | Hailperin co-operations — multiset boundary scaling | Inclusion-exclusion corners (alternating sign sequences / polyhedral vertex squeezing) | Polyhedral corner boundary coordinates / vertex inequality matrix | `idris2-Boole` 📄 `Hailperin.idr` |
