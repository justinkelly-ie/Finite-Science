...
| 🧮 **[Functional Probability](../idris2-Universe-Wiki/Library/Wiki/Finite-Science/Functional_Probability.md)** (Hehner) | Quantified mset $\{w \cdot [\text{valid}]\}$ | Total universe sum $\{\sum w \cdot [\text{all states}]\}$ | Hehner normalization $\overline{E} = E \cdot (\sum E)^{-1}$ | Min / max evaluation (calculational quantifiers $\land$ and $\lor$ replacing infinite logic) | `idris2-Boole` 📄 `FunctionalProbability.idr` |
...
| 🔬 **[Indivisible Stochastic Mechanics](../idris2-Universe-Wiki/Library/Wiki/Finite-Science/Indivisible_Mechanics.md)** (Barandes) | Indivisible configurations $\{p \cdot [\text{trajectory}]\}$ | Division time events $\{\sum p \cdot [\text{prior breaks}]\}$ | Non-Markovian transitions — amplitudes $\Theta$ to proportions $P$ | Stochastic correspondence (quantum systems without wavefunctions / $P = \vert\Theta\vert^2$) | `idris2-Universe` 📄 `IndivisibleHistory.idr` |
...
| ⚡ **[Electromagnetism](../idris2-Universe-Wiki/Library/Wiki/Finite-Science/Electromagnetism.md)** | Loop chain tracks $\{q \cdot [\text{Edge Loops}]\}$ | Volume element scale $\{\text{Cell}_\text{Volume}\}$ | Exterior calculus decay — discrete boundary maps | Gauge invariant closure (discrete curl $\Delta_\text{matrix} \to 0$ via Hodge splits / charge conservation) | `idris2-Universe` 📄 `DiscreteCalculus.idr` 📄 `GaugeOperator.idr` |
| 🕳️ **[Inversive Space Relativity](../idris2-Universe-Wiki/Library/Wiki/Finite-Science/Inversive_Space.md)** | Conic equation vexels $\{c \cdot [\text{Conic Coeffs}]\}$ | Inversion density $\{1 \cdot [Q_\text{Circle}^n]\}$ | Inversive chromogeometry — circle-reflection transforms | Conic edge splitting (Euler line & omega intersections / bending path bounds) | `idris2-Universe` 📄 `Twist.idr` |
