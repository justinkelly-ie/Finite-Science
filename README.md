...
| Logic / Physics | Numerator (MSetFraction.num) | Denominator (MSetFraction.den) | Transformation MSet | Routing Template MSet | Symmetric Bilinear Form (SBF) | Repos & Source Files |
|---|---|---|---|---|---|---|
| 🎛️ **[Singleton Repetition](https://github.com/justinkelly-ie/idris2-Universe-Wiki/blob/main/Library/Wiki/Finite-Science/Singleton_Repition.md)** (Algebra of Boole - Singleton Variant) | Singleton Bit-gate multiset $\{b \cdot [\text{state}]\}$ (size 0 or 1) | Strictly positive unit constant $\{1 \cdot [\text{BaseAnchor}]\}$ (size 1) | Boole Maxel (multiset of logic transition relations `SingRelation` / `Pixel` with `BF2` weights) | Logic SBFMset Maxel (multiset of wire connections `SingRelation` / `Pixel` over `Coord1D`) | `row1SBF` = `[((Wire, Wire), 1)]` (modulo-2 parity collapse/identity form) | `idris2-Boole`, `idris2-Multiset` (`BF2.idr`, `SingFraction.idr`, `Transformation.idr`, `SBFMset.idr`, `Sing.idr`, `Sing1.idr`) |
| 💻 **[Singleton Lifted Polynumbers](https://github.com/justinkelly-ie/idris2-Universe-Wiki/blob/main/Library/Wiki/Finite-Science/Singleton_Lifted.md)** (Arithmetic Circuits - Singleton Variant) *(Lifted Polynumbers is Deprecated)* | Singleton truth table coefficients $\{w \cdot [\text{state}]\}$ (size 0 or 1) | Strictly positive unit constant $\{1 \cdot [\text{BaseAnchor}]\}$ (size 1) | Boole Maxel (multiset of Boole-Möbius shift relations `SingRelation` / `Pixel` with `BoxInt` weights) | Positional SBFMset Maxel (multiset of positional scaling connections `SingRelation` / `Pixel` with `BoxInt` weights) | `row2SBF` = `[((Reg_i, Reg_i), 2^i)]` (positional binary scaling/carry form) | `idris2-Boole`, `idris2-Multiset` (`BoxInt.idr`, `SingFraction.idr`, `Transformation.idr`, `SBFMset.idr`, `Sing.idr`, `Sing1.idr`) |
| 🧮 **[Functional Probability](https://github.com/justinkelly-ie/idris2-Universe-Wiki/blob/main/Library/Wiki/Finite-Science/Functional_Probability.md)** (Hehner) | Quantified multiset (numerator box) $\{w \cdot [\text{valid}]\}$ (`Sing BoxInt v`) | Total universe sum (denominator box) $\{\sum w \cdot [\text{all states}]\}$ (`Sing1 BoxInt TrivialBase`) | Probability Maxel (normalised probability distribution of states) | Min/max evaluation (calculational quantifiers replacing infinite logic) | Normalizing inner product Maxel $[((State_i, State_i), 1)]$ ($\langle A, B \rangle = \sum A_i B_i$) | `idris2-Boole` 📄 `FunctionalProbability.idr` |
| 📉 **[Probability Bounds](https://github.com/justinkelly-ie/idris2-Universe-Wiki/blob/main/Library/Wiki/Finite-Science/Probability_Bounds.md)** (Hailperin) | Min/max corner bounds $\langle \text{Box}\_{\text{Min}}, \text{Box}\_{\text{Max}} \rangle$ (`MSetFraction` interval) | Universe constraints $\{\sum w \cdot [\text{bounds}]\}$ (probability bounds constraint multiset) | Hailperin Maxel (multiset of Möbius-inverted inclusion-exclusion boundary transitions) | Inclusion-exclusion corners (alternating sign sequences / polyhedral vertex squeezing) | Inclusion-exclusion boundary Maxel (polyhedral vertex inequality matrix) | `idris2-Boole` 📄 `Hailperin.idr` |
| 🔬 **[Indivisible Stochastic Mechanics](https://github.com/justinkelly-ie/idris2-Universe-Wiki/blob/main/Library/Wiki/Finite-Science/Indivisible_Mechanics.md)** (Barandes) | Indivisible configurations $\{p \cdot [\text{trajectory}]\}$ | Division time events $\{\sum p \cdot [\text{prior breaks}]\}$ | Non-Markovian transitions — amplitudes $\Theta$ to proportions $P$ | Stochastic correspondence (quantum systems without wavefunctions / $P = \vert\Theta\vert^2$) | Stochastic correspondence form (Barandes transition rules) | `idris2-Universe` 📄 `IndivisibleHistory.idr` |
| 📐 **[Rational Trigonometry](https://github.com/justinkelly-ie/idris2-Universe-Wiki/blob/main/Library/Wiki/Finite-Science/Rational_Trigonometry.md)** (Wildberger) | Triangle side msets $\{\Delta x^2, \Delta y^2\}$ | Hypotenuse field $\{1 \cdot [Q_\text{Hyp}]\}$ | Affine bilinear forms — dot-product symmetrics | Triple quad / spread laws (quadratic cross-multiplication over integers / no angles) | Symmetric dot product $A \cdot B = x_1 x_2 + y_1 y_2$ | `idris2-Chromogeometry` 📄 `Math.Chromogeometry` |
| 🔵 **[Blue Geometry](https://github.com/justinkelly-ie/idris2-Universe-Wiki/blob/main/Library/Wiki/Finite-Science/Blue_Geometry.md)** (Euclidean Space) | Blue metric mset $\{(\Delta x)^2 + (\Delta y)^2\}$ | Blue grid scale mset $\{\text{Grid}_\text{Density}\}$ | Blue maxel tensor ($I$) — standard identity convolution | Euclidean invariance (strictly positive spatial quadrances / rigid shapes) | Identity matrix $\begin{bmatrix} 1 & 0 \\ 0 & 1 \end{bmatrix}$ ($Q = x^2 + y^2$) | `idris2-Chromogeometry` 📄 `Math.Chromogeometry` (via `quadranceNL`) |
| 🔴 **[Red Geometry](https://github.com/justinkelly-ie/idris2-Universe-Wiki/blob/main/Library/Wiki/Finite-Science/Red_Geometry.md)** (Minkowski Relativity) | Red metric mset $\{(\Delta x)^2 - (\Delta y)^2\}$ | Red grid scale mset $\{\text{Grid}_\text{Density}\}$ | Red maxel tensor ($k$) — reflection matrix convolution | Minkowski relativity (light cones / invariant null lines where $Q_r = 0$) | Minkowski matrix $\begin{bmatrix} 1 & 0 \\ 0 & -1 \end{bmatrix}$ ($Q = x^2 - y^2$) | `idris2-Chromogeometry` 📄 `Math.Chromogeometry` (via `quadranceNL`) |
| 🟢 **[Green Geometry](https://github.com/justinkelly-ie/idris2-Universe-Wiki/blob/main/Library/Wiki/Finite-Science/Green_Geometry.md)** (Diagonal Shear) | Green metric mset $\{2(\Delta x)(\Delta y)\}$ | Green grid scale mset $\{\text{Grid}_\text{Density}\}$ | Green maxel tensor ($j$) — anti-diagonal transposition | Diagonal symmetry / shear (secondary relativistic alternative along the $y=x$ bounds) | Diagonal shear matrix $\begin{bmatrix} 0 & 1 \\ 1 & 0 \end{bmatrix}$ ($Q = 2xy$) | `idris2-Chromogeometry` 📄 `Math.Chromogeometry` (via `quadranceNL`) |
| 🌌 **[Universal Hyperbolic Geometry](https://github.com/justinkelly-ie/idris2-Universe-Wiki/blob/main/Library/Wiki/Finite-Science/Universal_Hyperbolic_Geometry.md)** | Homogeneous msets $\{w \cdot [x : y : z]\}$ | Projective frame field $\{1 \cdot [\text{Base Frame}]\}$ | Projective reconstructions — cross-ratio invariants | Duality matrix switches (perfect point-to-line flips across finite grids) | Projective incidence polarity matrix (duality switch form) | `idris2-Multiset` 📄 `ExtendedCosmology.idr` |
| ⚡ **[Electromagnetism](https://github.com/justinkelly-ie/idris2-Universe-Wiki/blob/main/Library/Wiki/Finite-Science/Electromagnetism.md)** | Loop chain tracks $\{q \cdot [\text{Edge Loops}]\}$ | Volume element scale $\{\text{Cell}_\text{Volume}\}$ | Exterior calculus decay — discrete boundary maps | Gauge invariant closure (discrete curl $\Delta_\text{matrix} \to 0$ via Hodge splits / charge conservation) | Hodge star bilinear operator (co-differential constraints) | `idris2-Universe` 📄 `DiscreteCalculus.idr` 📄 `GaugeOperator.idr` |
| 🕳️ **[Inversive Space Relativity](https://github.com/justinkelly-ie/idris2-Universe-Wiki/blob/main/Library/Wiki/Finite-Science/Inversive_Space.md)** | Conic equation vexels $\{c \cdot [\text{Conic Coeffs}]\}$ | Inversion density $\{1 \cdot [Q_\text{Circle}^n]\}$ | Inversive chromogeometry — circle-reflection transforms | Conic edge splitting (Euler line & omega intersections / bending path bounds) | Inversive circle-reflection bilinear form (conic incidence) | `idris2-Universe` 📄 `Twist.idr` |
| 🧬 **[Quantum Invariants](https://github.com/justinkelly-ie/idris2-Universe-Wiki/blob/main/Library/Wiki/Finite-Science/Quantum_Invariants.md)** | Spread vexel mset $\{c_n \cdot [s^n]\}$ | Hypotenuse scaling mset $\{1 \cdot [Q_\text{Hyp}^n]\}$ | Algebraic calculus — transition difference operators | Goh factorisation (decomposition of spread polynumbers into Fibonacci parts) | Goh factorization algebraic spread form | `idris2-Multiset` 📄 `SpreadPolynumber.idr` |
