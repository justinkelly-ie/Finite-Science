...
|---|---|---|---|---|---|
| 🎛️ **[Digital Repetition](../idris2-Universe-Wiki/Library/Wiki/Finite-Science/Digital_Repetition.md)** (Algebra of Boole) | BIT-GATE MULTISET $\{b \cdot [\text{state}]\}$ | THE UNIT CONSTANT $\{1 \cdot [\text{Base}]\}$ | MODULO-2 LOGIC FIELD Modulo-2 Bit Arithmetic | BOOLEAN RING EVALUATION (Exclusive OR / Linear Bit Flipping where $1+1=0$) | `idris2-Boole` 📄 `BF2.idr` 📄 `BitGate.idr` 📄 `BooleFraction.idr` |
| 💻 **[Lifted Polynombers](../idris2-Universe-Wiki/Library/Wiki/Finite-Science/Lifted_Polynombers.md)** (Arithmetic Circuits) | TRUTH TABLE COEFFICIENTS $\{w \cdot [\text{strings}]\}$ | THE UNIT CONSTANT $\{1 \cdot [\text{Base}]\}$ | BOOLE-MÖBIUS SHIFTS Upper-Triangular Matrix | POLYNOMIAL MAPPING (Algebraic Normal Form Lift / Idempotent $x^2=x$ Collapse) | `idris2-Boole` 📄 `BoxInt.idr` 📄 `IntPolynumber.idr` |
| 🧮 **[Functional Probability](../idris2-Universe-Wiki/Library/Wiki/Finite-Science/Functional_Probability.md)** (Hehner) | QUANTIFIED MSET $\{w \cdot [\text{valid}]\}$ | TOTAL UNIVERSE SUM $\{\sum w \cdot [\text{all\_states}]\}$ | HEHNER NORMALIZATION $\overline{E} = E \cdot (\sum E)^{-1}$ | MIN / MAX EVALUATION (Calculational Quantifiers $\land$ and $\lor$ replacing infinite logic) | `idris2-Boole` 📄 `Hehner.idr` |
| 📉 **[Probability Bounds](../idris2-Universe-Wiki/Library/Wiki/Finite-Science/Probability_Bounds.md)** (Hailperin) | MIN/MAX CORNER MSETS $\langle \text{Box}_\text{Min}, \text{Box}_\text{Max} \rangle$ | UNIVERSE CONSTRAINTS $\{\sum w \cdot [\text{bounds}]\}$ | HAILPERIN CO-OPERATIONS Multiset Boundary Scaling | INCLUSION-EXCLUSION CORNERS (Alternating Sign Sequences / Polyhedral Vertex Squeezing) | `idris2-Boole` 📄 `Hailperin.idr` |
| 🔬 **[Indivisible Stochastic Mechanics](../idris2-Universe-Wiki/Library/Wiki/Finite-Science/Indivisible_Mechanics.md)** (Barandes) | INDIVISIBLE CONFIGURATIONS $\{p \cdot [\text{trajectory}]\}$ | DIVISION TIME EVENTS $\{\sum p \cdot [\text{prior\_breaks}]\}$ | NON-MARKOVIAN TRANSITIONS Amplitudes $\Theta$ to Proportions $P$ | STOCHASTIC CORRESPONDENCE (Quantum Systems without Wavefunctions / $P = \vert\Theta\vert^2$) | `idris2-Universe` 📄 `IndivisibleHistory.idr` |
| 📐 **[Rational Trigonometry](../idris2-Universe-Wiki/Library/Wiki/Finite-Science/Rational_Trigonometry.md)** (Wildberger) | TRIANGLE SIDE MSETS $\{\Delta x^2, \Delta y^2\}$ | HYPOTENUSE FIELD $\{1 \cdot [Q_\text{Hyp}]\}$ | AFFINE BILINEAR FORMS Dot-Product Symmetrics | TRIPLE QUAD / SPREAD LAWS (Quadratic cross-multiplication over integers / No Angles) | `idris2-Chromogeometry` 📄 `Math.Chromogeometry` |
| 🔵 **[Blue Geometry](../idris2-Universe-Wiki/Library/Wiki/Finite-Science/Blue_Geometry.md)** (Euclidean Space) | BLUE METRIC MSET $\{(\Delta x)^2 + (\Delta y)^2\}$ | BLUE GRID SCALE MSET $\{\text{Grid}_\text{Density}\}$ | BLUE MAXEL TENSOR ($I$) Standard Identity Convolution | EUCLIDEAN INVARIANCE (Strictly positive spatial quadrances / Rigid Shapes) | `idris2-Chromogeometry` 📄 `Math.Chromogeometry` (via `quadranceNL`) |
| 🔴 **[Red Geometry](../idris2-Universe-Wiki/Library/Wiki/Finite-Science/Red_Geometry.md)** (Minkowski Relativity) | RED METRIC MSET $\{(\Delta x)^2 - (\Delta y)^2\}$ | RED GRID SCALE MSET $\{\text{Grid}_\text{Density}\}$ | RED MAXEL TENSOR ($k$) Reflection Matrix Convolution | MINKOWSKI RELATIVITY (Light Cones / Invariant Null Lines where $Q_r = 0$) | `idris2-Chromogeometry` 📄 `Math.Chromogeometry` (via `quadranceNL`) |
| 🟢 **[Green Geometry](../idris2-Universe-Wiki/Library/Wiki/Finite-Science/Green_Geometry.md)** (Diagonal Shear) | GREEN METRIC MSET $\{2(\Delta x)(\Delta y)\}$ | GREEN GRID SCALE MSET $\{\text{Grid}_\text{Density}\}$ | GREEN MAXEL TENSOR ($j$) Anti-Diagonal Transposition | DIAGONAL SYMMETRY / SHEAR (Secondary relativistic alternative along the $y=x$ bounds) | `idris2-Chromogeometry` 📄 `Math.Chromogeometry` (via `quadranceNL`) |
| 🌌 **[Universal Hyperbolic Geometry](../idris2-Universe-Wiki/Library/Wiki/Finite-Science/Universal_Hyperbolic_Geometry.md)** | HOMOGENEOUS MSETS $\{w \cdot [x : y : z]\}$ | PROJECTIVE FRAME FIELD $\{1 \cdot [\text{Base\_Frame}]\}$ | PROJECTIVE RECONSTRUCTIONS Cross-Ratio Invariants | DUALITY MATRIX SWITCHES (Perfect Point-to-Line Flips across finite grids) | `idris2-Multiset` 📄 `ExtendedCosmology.idr` |
| ⚡ **[Electromagnetism](../idris2-Universe-Wiki/Library/Wiki/Finite-Science/Electromagnetism.md)** | LOOP CHAIN TRACKS $\{q \cdot [\text{Edge\_Loops}]\}$ | VOLUME ELEMENT SCALE $\{\text{Cell}_\text{Volume}\}$ | EXTERIOR CALCULUS DECAY Discrete Boundary Maps | GAUGE INVARIANT CLOSURE (Discrete Curl $\Delta_\text{matrix} \to 0$ via Hodge splits / Charge Conservation) | `idris2-Universe` 📄 `DiscreteCalculus.idr` 📄 `GaugeOperator.idr` |
| 🕳️ **[Inversive Space Relativity](../idris2-Universe-Wiki/Library/Wiki/Finite-Science/Inversive_Space.md)** | CONIC EQUATION VEXELS $\{c \cdot [\text{Conic\_Coeffs}]\}$ | INVERSION DENSITY $\{1 \cdot [Q_\text{Circle}^n]\}$ | INVERSIVE CHROMOGEOMETRY Circle-Reflection Transforms | CONIC EDGE SPLITTING (Euler Line & Omega Intersections / Bending path bounds) | `idris2-Universe` 📄 `Twist.idr` |
| 🧬 **[Quantum Invariants](../idris2-Universe-Wiki/Library/Wiki/Finite-Science/Quantum_Invariants.md)** | SPREAD VEXEL MSET $\{c_n \cdot [s^n]\}$ | HYPOTENUSE SCALING MSET $\{1 \cdot [Q_\text{Hyp}^n]\}$ | ALGEBRAIC CALCULUS Transition Difference Operators | GOH FACTORISATION (Decomposition of Spread polynumbers into Fibonacci parts) | `idris2-Multiset` 📄 `SpreadPolynumber.idr` |
