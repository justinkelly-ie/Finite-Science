...
||| A Multiset (UnaryMultiset) is a linear collection of atoms.
...
||| Runtime size.
public export
size : UnaryMultiset a -> Nat
size Zero = 0
size (Add x xs) = 1 + size xs

public export
deleteFirstEq : Eq a => a -> UnaryMultiset a -> Maybe (UnaryMultiset a)
...
public export
implementation Eq a => Eq (UnaryMultiset a) where
  Zero == Zero = True
  Zero == (Add _ _) = False
  (Add _ _) == Zero = False
...
||| Linear count: Structurally consumes the UnaryMultiset () to produce a runtime Integer.
public export
countMSet : (1 _ : UnaryMultiset ()) -> Integer
countMSet xs = go 0 xs
  where
    go : Integer -> (1 _ : UnaryMultiset ()) -> Integer
    go acc Zero = acc
...
record SizeProof a where
  ...
  1 n : UnaryMultiset ()
  1 original : UnaryMultiset a
  1 reconstructed : UnaryMultiset a

||| Linear size: Returns a proof containing the count and the multiset.
public export
sizeL : (LComonoid a) => (1 _ : UnaryMultiset a) -> SizeProof a
...
public export
implementation (LConsumable a) => LConsumable (UnaryMultiset a) where
  lconsume Zero = ()
  lconsume (Add x xs) = case lconsume x of () => lconsume xs

public export
implementation (LComonoid a) => LComonoid (UnaryMultiset a) where
  lcounit Zero = ()
...
public export
implementation (Show a) => Show (UnaryMultiset a) where
  show Zero = "[]"
  show (Add x xs) = "[" ++ show x ++ showRest xs ++ "]"
    where
      showRest : UnaryMultiset a -> String
      showRest Zero = ""
      showRest (Add y ys) = ", " ++ show y ++ showRest ys

public export
implementation (LEq a) => LEq (UnaryMultiset a) where
...
public export
(++) : (1 _ : UnaryMultiset a) -> (1 _ : UnaryMultiset a) -> UnaryMultiset a
Zero ++ ys = ys
(Add x xs) ++ ys = Add x (xs ++ ys)

||| Multiset Sum (Box Arithmetic)
public export
add : (1 _ : UnaryMultiset a) -> (1 _ : UnaryMultiset a) -> UnaryMultiset a
add xs ys = xs ++ ys

public export
implementation Semigroup (UnaryMultiset a) where
  Zero <+> ys = ys
  (Add x xs) <+> ys = Add x (xs <+> ys)

public export
implementation Monoid (UnaryMultiset a) where
  neutral = Zero

||| Non-linear map helper for auditing.
public export
map0 : (a -> b) -> UnaryMultiset a -> UnaryMultiset b
map0 f Zero = Zero
map0 f (Add x xs) = Add (f x) (map0 f xs)

||| Linear Map
public export
mapMSetL : ((1 _ : a) -> b) -> (1 _ : UnaryMultiset a) -> UnaryMultiset b
mapMSetL f Zero = Zero
mapMSetL f (Add x xs) = Add (f x) (mapMSetL f xs)

||| Flattening / Concatenation (Box Arithmetic Sigma)
public export
sigma : (1 _ : UnaryMultiset (UnaryMultiset a)) -> UnaryMultiset a
sigma Zero = Zero
sigma (Add x xs) = x ++ sigma xs

||| Linear Cartesian Product mapping (internal helper for convolution)
public export
mulLHelper : (LComonoid a) => ((1 _ : a) -> (1 _ : b) -> c) -> (1 _ : a) -> (1 _ : UnaryMultiset b) -> UnaryMultiset c
...
public export
mulL : (LComonoid a, LComonoid b) => ((1 _ : a) -> (1 _ : b) -> c) -> (1 _ : UnaryMultiset a) -> (1 _ : UnaryMultiset b) -> UnaryMultiset c
...
public export
cartesianProduct : (LComonoid a, LComonoid b) => (1 _ : UnaryMultiset a) -> (1 _ : UnaryMultiset b) -> UnaryMultiset (LPair a b)
cartesianProduct xs ys = mulL (\x, y => Builtin.(#) x y) xs ys

||| Box Arithmetic Multiplication
public export
mul : LComonoid a => (1 _ : UnaryMultiset (UnaryMultiset a)) -> (1 _ : UnaryMultiset (UnaryMultiset a)) -> UnaryMultiset (UnaryMultiset a)
mul a b = mulL (\x, y => add x y) a b

||| Box Arithmetic Carret Product
public export
carret : LComonoid a => (1 _ : UnaryMultiset (UnaryMultiset (UnaryMultiset a))) -> (1 _ : UnaryMultiset (UnaryMultiset (UnaryMultiset a))) -> UnaryMultiset (UnaryMultiset (UnaryMultiset a))
carret a b = mulL (\x, y => mul x y) a b

||| Alpha Power (Box Arithmetic)
public export
alphaPow : (1 _ : a) -> UnaryMultiset a
alphaPow x = Add x Zero

public export
fromNat : Nat -> UnaryMultiset (UnaryMultiset ())
fromNat Z = Zero
fromNat (S k) = Add Zero (fromNat k)

||| Less-Than-Or-Equal for LNat (UnaryMultiset ())
public export
lLTE : (1 _ : UnaryMultiset ()) -> (1 _ : UnaryMultiset ()) -> LPair Bool (LPair (UnaryMultiset ()) (UnaryMultiset ()))
...
public export
truncate : Nat -> (1 _ : UnaryMultiset (UnaryMultiset (UnaryMultiset ()))) -> UnaryMultiset (UnaryMultiset (UnaryMultiset ()))
...
public export
0 squash : Eq a => UnaryMultiset a -> UnaryMultiset a
squash m = m

||| Linear Squash: consumes the multiset.
public export
squashL : (Eq a, LConsumable a, LComonoid a) => (1 _ : UnaryMultiset a) -> UnaryMultiset a
squashL Zero = Zero
...
public export
fromList : (1 _ : List a) -> UnaryMultiset a
fromList [] = Zero
...
public export
count : (Eq a) => a -> UnaryMultiset a -> Nat
count x Zero = 0
count x (Add y ys) = 
  if x == y then 1 + count x ys else count x ys

||| Linear count: Consumes the multiset and the target element, returning them both.
||| Returns an UnaryMultiset () (LNat) to maintain strict structural linearity.
public export
countL : (LEq a) => (1 _ : a) -> (1 _ : UnaryMultiset a) -> LPair (UnaryMultiset ()) (LPair a (UnaryMultiset a))
...
public export
0 moebiusWeight : (totalSet : UnaryMultiset a) -> (subSet : UnaryMultiset a) -> Integer
moebiusWeight s t = 
...
public export
implementation (Cast a Nat) => Cast (UnaryMultiset a) (UnaryMultiset Nat) where
  cast Zero = Zero
